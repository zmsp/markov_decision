...
parser.add_argument('-e', '--environment', default='FrozenLake-v1',
...
def main():
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(max_step):
            ...
            next_s, r, t, done, info = env.step(curr_a)
            ...
            if algorithm_type == 'sarsa':
                delta = r + gamma * q_value[next_s, int(next_a)] - q_value[curr_s, int(curr_a)]
            ...
            q_value[curr_s, int(curr_a)] += alpha * delta
            ...
            if done:
                if policy_type == 'softmax':
                    print
                    ("Episode: {0}\t Steps: {1:>4}\tCumuR: {2:>5.2f}\tTermR: {3}\tBeta: {4:.3f}".format(i_episode,
                                                                                                       i_step, cumu_r,
                                                                                                       r, beta))
                    history.append([i_episode, i_step, cumu_r, r, beta])
                elif policy_type == 'epsilon_greedy':
                    print ( "Episode: {0}\t Steps: {1:>4}\tCumuR: {2:>5.2f}\tTermR: {3}\tEpsilon: {4:.3f}".format(i_episode,
                                                                                                          i_step,
                                                                                                          cumu_r, r,
                                                                                                          epsilon))
    ...
    # Number of steps
    ax[0, 0].plot(history[:, 0], history[:, 1], '.', label="Values")
    ax[0, 0].set_xlabel('Episode')
    ax[0, 0].set_ylabel('Number of steps')
    ax[0, 0].plot(history[window_size - 1:, 0], running_average(history[:, 1], window_size), label="Average")
    ax[0, 0].legend()
    # Cumulative reward
    ax[0, 1].plot(history[:, 0], history[:, 2], '.', label="Values")
    ax[0, 1].set_xlabel('Episode')
    ax[0, 1].set_ylabel('Cumulative rewards')
    ax[0, 1].plot(history[window_size - 1:, 0], running_average(history[:, 2], window_size), label="Average")
    # Terminal reward
    ax[1, 0].plot(history[:, 0], history[:, 3], '.', label="Values")
    ax[1, 0].set_xlabel('Episode')
    ax[1, 0].set_ylabel('Terminal rewards')
    ax[1, 0].plot(history[window_size - 1:, 0], running_average(history[:, 3], window_size), label="Average")
    # Epsilon/Beta
    ax[1, 1].plot(history[:, 0], history[:, 4], '.', label="Values")
